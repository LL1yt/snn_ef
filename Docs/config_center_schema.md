...
_Цель:_ единый конфиг для обоих исследовательских модулей. ConfigCenter читает YAML, валидирует значения, снапшот передается подсистемам без «горячих» правок. Все ключи обязательны, если не указано иное.
...
- Пути относительные рабочему каталогу; ConfigCenter нормализует и создает директории.
...
- Логи: override может ссылаться лишь на существующий `process_id`.
