...
- базовый бюджет параметров SNN — 512(если 512 слишком мало, то манимально возможное для такой архитектуры), задаётся в конфиге (`snn.parameter_count`) и может расширяться для экспериментов. параметры общие для всех SNN
...
- `RouterLogger.swift`: события `router.step`, `router.spike`, `router.output`.
